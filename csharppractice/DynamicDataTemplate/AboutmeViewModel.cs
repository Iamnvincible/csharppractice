...
namespace DynamicDataTemplate
{
    ...
    {
        public ObservableCollection<MyNotification> Items { get; private set; } = new ObservableCollection<MyNotification>();
            ...
            if (status != "200")
            {
                foreach (var item in jo.GetNamedArray("data", new JsonArray()))
                {
                    if (item.ValueType == JsonValueType.Object)
                    {
                        ...
                        var s = (MyNotification)ser.ReadObject(stream);
                        Items.Add(s);
                    }
                }
            }
        ...
        private async Task<string> GetJsonFromFile()
        {
            ...
            Uri datauri = new Uri("ms-appx:///samplenoti.json");
            //文件读写可能会有异常
            try
            {
                //获取文件
